...
conservera nacional, con cuatro etapas en serie:

\begin{center}
\texttt{cola} $\to$ FILETEADO $\to$ \texttt{cola} $\to$ \textbf{ENVASADO} $\to$
\texttt{cola} $\to$ SELLADO $\to$ \texttt{cola} $\to$ ESTERILIZACIÓN $\to$ listos
\end{center}
...
\textbf{cambian de escala por motivos distintos}. El nodo B es el único que se
...
quedan libres, en vez de recibirlo asignado por una pieza intermedia.
...
sola operación indivisible}. Redis, al ser monohilo en la atención de
comandos, garantiza que cada elemento se entrega a exactamente una réplica
...
la \emph{espera}. El experimento 2 lo ilustra: la etapa señalada como cuello
(esterilización, ciclo 7~s) tiene un ciclo \textbf{más corto} que envasado
(12~s) — un criterio por tiempo de servicio jamás la habría encontrado.
...
propósito. Varios contenedores escriben la misma base: se usa WAL,
\texttt{busy\_timeout} y una conexión por operación.
...
60~s. La llegada (0{,}20 lotes/s) supera la capacidad de envasado con una
réplica (1/12 = 0{,}083) y la de esterilización (1/7 = 0{,}143), pero no la de
fileteado (0{,}25) ni sellado (0{,}33). Datos crudos en
...
normales. Un round-robin habría repartido 10/10/10 sin importar la velocidad.
Nadie asignó ese reparto: emergió de que cada réplica reclama trabajo solo al
quedar libre.
...
congestión recorre la línea como una ola (fileteado $\to$ envasado $\to$
esterilización) y a los \textbf{278~s} las cuatro etapas están de vuelta en
\texttt{normal} \textbf{sin intervención alguna}: las esperas viejas salieron
de la ventana móvil. Detalle en \texttt{experimentos/resultados/exp5-saturacion.txt}.
...
Sin pasos manuales, sin instalar nada más que Docker. La prueba se ejecutó
...
muda}, y de que medir espera (no servicio) es lo que permite verlo en vivo.
