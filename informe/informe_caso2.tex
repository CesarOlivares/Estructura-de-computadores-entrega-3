...
ficticia de jurel en lata, con cuatro etapas en serie separadas por colas de
...
formato). El sistema es una arquitectura
...
cuello de botella se desplaza de envasado a esterilización y el lead
time cae un 21~\%; una tercera réplica ya casi no mejora nada, porque el límite
se mudó de etapa.

% =============================================================================
\section{Conceptos y vocabulario}
\label{sec:vocabulario}

Este informe usa vocabulario de la planta y de sistemas distribuidos y conviene fijar los dos antes de empezar, porque
varios términos se parecen y significan cosas diferentes.

\subsection{Los tres tiempos, que no son lo mismo}

La distinción entre estos tres es el eje de todo el análisis. Confundirlos lleva
a diagnosticar mal:

\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \textbf{Tiempo de ciclo:} cuánto tarda una etapa en procesar un lote
        cuando ya lo tiene en la mano. Es una propiedad de la máquina: la
        autoclave tarda 7~s y no hay forma de apurarla.
  \item \textbf{Tiempo de espera:} cuánto tiempo pasa un lote \emph{haciendo
        fila} antes de que una etapa lo tome. Es una propiedad de la carga, no
        de la máquina, y es la señal que usamos para detectar atascos.
  \item \textbf{\emph{Lead time}:} el total, desde que se
        crea la orden hasta que el lote sale terminado de la última etapa.
        Suma todas las esperas y todos los ciclos del recorrido. Es lo que
        percibe quien encarga la producción.
\end{itemize}

Un tiempo de ciclo largo significa que la tarea es lenta. Un tiempo de espera
que crece significa que la etapa no da abasto. Son cosas distintas, y
solo la segunda es un \textbf{cuello de botella}: la etapa a la que le llega
trabajo más rápido de lo que puede sacarlo, y frente a la cual la fila se
alarga sin parar.

\subsection{Vocabulario de sistemas distribuidos}

\begin{table}[H]
\centering
\footnotesize
\begin{tabular}{@{}L{3.05cm} L{12.15cm}@{}}
\toprule
\textbf{Término} & \textbf{Qué significa en este informe} \\
\midrule
\textbf{Servicio} &
Un programa que corre por su cuenta y ofrece algo a los demás a través de la
red, en vez de ser una función que se llama dentro del mismo programa. \\
\addlinespace[2pt]
\textbf{Réplica} &
Una copia idéntica de un servicio corriendo al mismo tiempo que las otras, para
repartirse el trabajo entre todas. \emph{Escalar} una etapa es levantar más
réplicas de ella. Ninguna sabe cuántas hermanas tiene. \\
\addlinespace[2pt]
\textbf{Nodo lógico} &
Un grupo de servicios que conviene mantener juntos porque escalan o fallan por
el mismo motivo. Es una agrupación de diseño: no obliga a usar máquinas
físicas distintas, pero deja listo el corte por si se necesitaran. \\
\addlinespace[2pt]
\textbf{API} &
\emph{Interfaz de programación de aplicaciones}: el conjunto de operaciones
que un servicio ofrece a los demás, con su formato de entrada y de salida
fijado de antemano. Es el contrato entre programas. \\
\addlinespace[2pt]
\textbf{REST y JSON} &
La convención que seguimos para esas operaciones: se piden por direcciones web
(\texttt{/ordenes}, \texttt{/metricas}) usando verbos HTTP —\texttt{GET} para
consultar, \texttt{POST} para crear— y los datos viajan como texto en formato
JSON, que es legible tanto para una persona como para un programa. \\
\addlinespace[2pt]
\textbf{Código HTTP} &
El número con el que una respuesta dice cómo le fue: \texttt{200} y
\texttt{201} bien, \texttt{404} no existe, \texttt{422} los datos enviados no
son válidos, \texttt{503} el servicio no puede atender ahora. \\
\addlinespace[2pt]
\textbf{Cola} &
Una fila de trabajo pendiente compartida: unos programas dejan tareas por un
extremo y otros las sacan por el otro, sin conocerse entre sí. \\
\addlinespace[2pt]
\textbf{Operación atómica} &
Una operación que ocurre entera o no ocurre: no existe ningún instante
intermedio en el que otro programa pueda ver el sistema a medio hacer. Es la
propiedad que hace que sacar un lote de una cola sea seguro. \\
\addlinespace[2pt]
\textbf{Condición de carrera} &
El error que aparece cuando dos programas actúan sobre lo mismo al mismo
tiempo y el resultado depende de cuál llegó primero. Aquí se manifiesta como
dos réplicas envasando el mismo lote. \\
\addlinespace[2pt]
\textbf{\emph{Round-robin}} &
La forma más simple de repartir trabajo: por turnos fijos y en orden —uno para
ti, uno para ti, uno para ti— sin mirar si alguien está ocupado o es más lento.
Es el contraste contra el que medimos nuestro reparto. \\
\addlinespace[2pt]
\textbf{Ventana móvil} &
Mirar solo lo ocurrido en los últimos $N$ segundos en vez de todo el
histórico, para que una congestión antigua y ya resuelta no siga contaminando
el diagnóstico de ahora. \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Las herramientas concretas}
...
\begin{tabular}{@{}L{3.05cm} L{12.15cm}@{}}
\toprule
\textbf{Herramienta} & \textbf{Qué hace y por qué está aquí} \\
\midrule
\textbf{Docker} &
Empaqueta un programa junto con todo lo que necesita para correr —intérprete,
librerías, configuración— en una unidad llamada \emph{contenedor}, que se
comporta igual en cualquier máquina. Es lo que hace que el sistema levante en
un computador que no tiene Python ni Redis instalados. \\
\addlinespace[2pt]
\textbf{Imagen y contenedor} &
La \emph{imagen} es la plantilla; el \emph{contenedor} es una ejecución
concreta de esa plantilla. Nuestras cuatro etapas son cuatro contenedores de
\textbf{la misma} imagen, con configuración distinta. \\
\addlinespace[2pt]
\textbf{Docker Compose} &
Describe en un solo archivo todos los contenedores del sistema y cómo se
conectan, de modo que levantar el conjunto completo sea un comando
(\texttt{docker compose up}) y no una secuencia de pasos manuales. \\
\addlinespace[2pt]
\textbf{Volumen} &
Un espacio de disco que vive fuera del contenedor, para que los datos
sobrevivan cuando el contenedor se apaga o se reemplaza. \\
\addlinespace[2pt]
\textbf{\emph{Healthcheck}} &
Una comprobación periódica y automática que Docker le hace a cada contenedor
para saber si sigue vivo. \\
\addlinespace[2pt]
\textbf{Redis} \cite{redis} &
Una base de datos que guarda todo en memoria, lo que la hace muy rápida.
Atiende los comandos de uno en uno, y esa propiedad es justamente la que
usamos para coordinar las réplicas: la usamos como cola de trabajo, no como
almacén permanente. \\
\addlinespace[2pt]
\textbf{\texttt{BRPOP}} &
El comando de Redis que saca el último elemento de una cola y lo entrega en
una sola operación indivisible; si la cola está vacía, se queda esperando en
vez de responder que no hay nada. \\
\addlinespace[2pt]
\textbf{SQLite} &
Una base de datos que vive en un único archivo, sin servidor aparte que
administrar. Guarda lo que tiene que sobrevivir a un apagado: las órdenes y su
historial de eventos. \\
\addlinespace[2pt]
\textbf{FastAPI} &
La librería de Python con la que están escritos los dos servicios que exponen
API. Valida sola los datos que llegan y genera documentación navegable de sus
propias operaciones. \\
\addlinespace[2pt]
\textbf{Streamlit} &
La librería de Python con la que está hecho el tablero. Permite construir una
interfaz web desde código Python, sin escribir HTML ni JavaScript. \\
\bottomrule
\end{tabular}
\end{table}
...
La agrupación no es arbitraria: cada ncodo reúne responsabilidades que
ambian de escala por motivos distintos, el criterio clásico para
...
\textbf{Por qué mantener dos o más nodos:} separa dominios de falla y de
recursos — la
...
La forma habitual de repartir trabajo entre réplicas es poner un
balanceador delante de ellas: una pieza intermedia que recibe cada
tarea y decide a qué réplica mandársela. Decidimos implementar el balanceo como
...
\begin{enumerate}[leftmargin=*]
  ...
  \item El reparto emergente es proporcional a la capacidad real de cada
        réplica, sin conocer cuántas hay.
\end{enumerate}

El balanceador existe, pero es la cola: el punto único por
...
consultas de estado; para el caso, el reparto por demanda es
estrictamente superior en este caso.

\subsection{Coordinación y condición de carrera: provocarla antes de resolverla}
\label{sec:carrera}

El problema de coordinación —que dos réplicas no tomen la misma orden— se
abordó en dos pasos deliberados, ambos preservados en el historial de commits:

\textbf{Versión no atómica (Incluyendo el defecto a propósito).} Reclamar
una orden en dos operaciones: mirar la cola (\texttt{LRANGE}) y después sacar
el elemento (\texttt{LREM}). Entre ambas hay una ventana en la que otra réplica ve
...
\begin{tikzpicture}[
  ...
  \node (tit1) [font=\scriptsize\bfseries, text=usachdark] at (0,2.1) {Reclamo no atómico: mirar y sacar (2 pasos)};
  ...
  \node (tit2) [font=\scriptsize\bfseries, text=usachdark] at (7.6,2.1) {Extracción atómica: \texttt{BRPOP} (1 paso)};
...
Se prefirió la operación atómica de cola por sobre un contador distribuido porque elimina la
ventana en vez de administrarla: no hay TTL que expire a mitad de un ciclo ni
liberación de contador que se pueda olvidar. La función de un ``contador
distribuido'' la cumple el mismo Redis que serializa las extracciones.

\subsection{Criterio de cuello de botella:}
\label{sec:criterio}
...
Un tiempo de servicio alto solo dice que la tarea es larga; el atasco es
trabajo llegando más rápido de lo que la etapa lo drena y eso se manifiesta en
la espera, la misma relación entre llegadas, capacidad y fila que
formaliza la ley de Little \cite{little} y que la teoría de las restricciones
usa para localizar la limitante de un proceso \cite{goldratt}. El experimento
2 lo ilustra con datos (Figura \ref{fig:espera_vs_servicio}): la etapa señalada
como cuello (esterilización, ciclo 7~s) tiene un ciclo más corto que
...
El tablero (Streamlit, puerto 8501) tiene cuatro piezas, más el histórico:
\textbf{(1)} timeline de
...
órdenes desde la interfaz (\texttt{POST /ordenes}); y \textbf{(5)} gráfico histórico de la espera por etapa.
La UI no calcula ninguna métrica: las pide a \texttt{metrics-api}. Regla visual: el color marca \emph{importancia}, no identidad — lo normal se dibuja
en gris y el color fuerte queda reservado para advertencia y crítico, siempre acompañado de icono y texto.
...
lo que anticipa los resultados.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{1 réplica:} envasado acumula 14 lotes en cola y espera de
        67{,}8~s; el resto de la línea espera $<1$~s.
...
Ráfaga de 25 órdenes de golpe (envasado $\times$2) cada 15~s: a los 17~s la primera etapa pasa a
...
\texttt{404} (orden inexistente), \texttt{503} (dependencia caída). La UI nunca muestra un traceback: ante cualquier falla consulta
...
resolución DNS (Docker elimina el nombre del contenedor detenido). La solución fue un
...
Resultado medido: con Redis caído pasó de colgarse 46~s
a responder \texttt{503} en \textbf{2{,}1~s}, y \texttt{/metricas} responde
\texttt{200} en 2{,}0~s con \texttt{en\_cola: null}.
...
Este proyecto, deliberadamente,
\textbf{detecta y explica} las fallas en vez de enmascararlas (sin reintentos). Si se necesitara operar esto en producción, la
arquitectura ya deja los puntos de corte correctos:

\begin{itemize}[leftmargin=*]
  \item \textbf{Estaciones (nodo B):} ya son resilientes en lo esencial — sin
        estado propio, reconectan solas y
...
La división en nodos de \ref{sec:nodos} es exactamente la que hace estas mejoras locales: cada mejora toca un nodo sin rediseñar los demás.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{La condición de carrera hubo que fabricarla para verla.} Con
        el reclamo no atómico, la ventana entre mirar y sacar es de
  ...
  \item \textbf{El cuello de botella depende del patrón de llegada.} Inyectar
        ...
        propiedad de las colas cambió el protocolo experimental.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Nada obligaba a persistir un conjunto concreto de datos: decidimos
        guardar órdenes y eventos, con las métricas siempre derivadas.
  \item El criterio de cuello de botella y sus umbrales no vienen dados por el
        problema: los de \ref{sec:criterio} son decisión nuestra.
\end{enumerate}

% =============================================================================
\section{Conclusiones}

El sistema resuelve el caso y sus afirmaciones
...
órdenes, contra duplicados reproducibles de la versión no atómica), el criterio de
...
\texttt{05efcc9} la reproduce con el reclamo no atómico y \texttt{3f1c79b} la
