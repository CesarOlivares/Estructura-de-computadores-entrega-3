...
% ── 17 · SATURACIÓN ──────────────────────────────────────────────────────────
\begin{frame}
\titulo{Saturación y recuperación automática}{Qué hace el sistema ante una ráfaga: 25 órdenes de golpe, sin intervención posterior}
\begin{center}
\includegraphics[width=0.72\linewidth]{fig_saturacion.pdf}
\end{center}
\begin{itemize}
  \item Cada celda del gráfico es una llamada al diagnóstico de la API de métricas (la función de cuello recién vista), hecha \textbf{cada 15 s} durante la prueba.
  \item A los 32 s el sistema declara \textbf{crítico} solo; la congestión recorre la línea como una ola. A los \textbf{278 s} todo vuelve a normal \textbf{sin intervención}: las esperas viejas salieron de la ventana móvil.
\end{itemize}
\end{frame}

% ── 18 · ERRORES ─────────────────────────────────────────────────────────────
\begin{frame}
\titulo{Manejo de errores}{El error que provocamos, su causa real y la solución — medido antes y después}
\begin{columns}[T]
\column{0.30\textwidth}
\cifra{46 s}{antes: la API quedaba muda con Redis detenido}
\column{0.30\textwidth}
\cifra{2,1 s}{después: respuesta 503 con mensaje que nombra al servicio caído}
\column{0.34\textwidth}
\vspace{0.35cm}
{\footnotesize\color{grisazul}Además: \texttt{201} creación · \texttt{422} validación · \texttt{404} inexistente · \texttt{503} dependencia caída — verificados contra el sistema corriendo.}
\end{columns}
\vspace{0.2cm}
\begin{itemize}
  \item \textbf{El error}: detuvimos Redis con el sistema corriendo; crear una orden dejaba a la API 46 s sin responder nada.
  \item \textbf{El diagnóstico}: el cuelgue no estaba en la conexión sino en la \textbf{resolución del nombre} (DNS) — al detener un contenedor, Docker elimina su nombre y la búsqueda tarda $\sim$45 s en rendirse, fuera del alcance del timeout de conexión.
  \item \textbf{La solución}: un plazo duro de 2 s sobre la operación completa; si no hay respuesta, se devuelve de inmediato un 503 que \textbf{nombra al servicio que falta}, y la interfaz nunca muestra un traceback.
\end{itemize}
\end{frame}

% ── 19 · PERSISTENCIA Y REPRODUCIBILIDAD ─────────────────────────────────────
\begin{frame}[fragile]
\titulo{Persistencia y reproducibilidad}{Dos tablas, nada guardado dos veces; despliegue completo con un comando}
\begin{columns}[T]
\column{0.52\textwidth}
\begin{itemize}
  \item SQLite en un volumen de Docker, dos tablas: \texttt{ordenes} y \texttt{eventos} (12 eventos por orden completa).
  ...
  \item La base sobrevive a apagar y levantar el sistema (verificado).
\end{itemize}
\column{0.44\textwidth}
{\footnotesize\color{grisazul}La prueba de reproducibilidad: clonar el repositorio en una carpeta vacía y levantar todo — nada más que Docker instalado:}\\[2pt]
\begin{beamercolorbox}[sep=8pt, rounded=true]{tarjetabox}
...
\begin{itemize}
  \item \textbf{Estaciones:} sin estado propio, reconectan y mueren sin corromper nada. Pendiente: si una réplica muere a mitad de ciclo, su lote se pierde de la cola; el patrón estándar es moverlo a una cola «en proceso» al reclamarlo (comando \texttt{BLMOVE} de Redis) y devolverlo si la réplica deja de dar señales de vida.
  \item \textbf{Redis:} punto único de falla del reparto $\to$ una réplica de Redis con conmutación automática (Redis Sentinel), o un broker de mensajes con confirmación de entrega (p.\,ej.\ Kafka): un mensaje no confirmado vuelve a entregarse solo.
  \item \textbf{SQLite:} con más escritores o conmutación, migrar a MySQL/PostgreSQL cambiando solo el módulo de base de datos (\texttt{bd.py}) de cada servicio.
  \item \textbf{APIs y tablero:} sin estado; dos instancias tras un balanceador HTTP quedan en alta disponibilidad — los chequeos de salud ya existentes son su insumo.
\end{itemize}
\vspace{0.2cm}
