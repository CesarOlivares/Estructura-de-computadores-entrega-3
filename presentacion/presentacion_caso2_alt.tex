...
% Condensada para las cifras grandes: Roboto Condensed, del árbol de TeX Live.
%
% Aquí NO se usa Bahnschrift, que sería la condensada natural en Windows:
% es una fuente variable en un solo archivo, y dvipdfmx no sabe incrustarla —
% la lee como si fuera una colección TTC y aborta la compilación con
% «Invalid TTC index / Invalid font: -1». Roboto Condensed es estática, da el
% mismo aire DIN, y al venir del árbol de TeX Live sale idéntica acá y en
% Overleaf.
...
\begin{frame}[plain]
\vfill
% Cada bloque cierra su propio \par dentro del grupo. Si no, el interlineado
% grande del titular no alcanza a aplicarse y las líneas se montan una encima
% de otra; y con \raggedright el titular no se justifica cuando ocupa casi
% todo el ancho.
\hspace{1.5cm}\begin{minipage}{0.86\linewidth}
\raggedright
{\antetitulo\scriptsize\color{tealu}\MakeUppercase{Estructura de Computadores y Sistemas Distribuidos}\par}
\vspace{18pt}
{\ligera\fontsize{28}{34}\selectfont\color{white}Línea de producción con\\ balanceo dinámico de carga\par}
\vspace{14pt}
{\color{tealu}\rule{2.6cm}{1.8pt}\par}
\vspace{14pt}
{\small\color{grilla}Un sistema distribuido que simula una planta conservera, detecta su cuello\\ de botella en vivo y demuestra qué pasa cuando se escala la etapa lenta\par}
\vspace{22pt}
{\small\color{white}\textbf{César Olivares · Simón García-Huidobro}\par}
\vspace{5pt}
{\footnotesize\color{suave}Evaluación 3 · Caso 2 · ECSD 2026-1 · Profesor: Daniel Calderón R. · 26 de agosto de 2026\par}
\end{minipage}
\vfill
\end{frame}}
...
% ── Arquitectura ─────────────────────────────────────────────────────────────
\begin{frame}[fragile]
\lamina{Arquitectura}{Tres nodos y una sola imagen de estación}
...
\begin{tikzpicture}[node distance=0.4cm and 0.85cm]
  \node (na) [nodo, minimum width=3.2cm, minimum height=1.55cm,
  ...
  \node (nb) [nodo, right=of na, minimum width=3.7cm, minimum height=1.55cm,
  ...
  \node (nc) [nodo, right=of nb, minimum width=3.2cm, minimum height=1.55cm,
...
\begin{itemize}
  \item \textbf{B} se replica bajo carga; \textbf{C} guarda el estado compartido; \textbf{A} escala con los usuarios.
  \item Las cuatro estaciones son \textbf{el mismo código}, configurado por variables de entorno.
\end{itemize}
...
\end{center}
\begin{itemize}
  \item Con 1 réplica, envasado acumula \textbf{67,8 s} de espera; el resto de la línea, casi ociosa.
  \item Con 2, el cuello \textbf{se muda a esterilización}. Con 3, envasado baja a 0,7 s\dots{} y el cuello no se mueve de ahí.
\end{itemize}
\end{frame}
